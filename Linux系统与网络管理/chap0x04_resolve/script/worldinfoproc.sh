...
function countrage()
{
    age=$(awk -F '\t' '{print $6}' Linux系统与网络管理/chap0x04_resolve/script/worldcupplayerinfo.tsv)
    sum_agelt20=0
    sum_ageb20to30=0
    sum_agegt30=0
    sum_total=0
...
function countposi()
{
  positions=$(awk -F '\t' '{print $5}' Linux系统与网络管理/chap0x04_resolve/script/worldcupplayerinfo.tsv) 
...
function countname()
{
        namelength=$( awk -F '\t' '{print length($9)}' Linux系统与网络管理/chap0x04_resolve/script/worldcupplayerinfo.tsv)
	longestnamelen=0
	shortestnamelen=103
	...
	longestname=$(awk -F '\t' '{if (length($9)=='$longestnamelen') {print $9} }' Linux系统与网络管理/chap0x04_resolve/script/worldcupplayerinfo.tsv)
	shortestname=$(awk -F '\t' '{if (length($9)=='$shortestnamelen') {print $9} }' Linux系统与网络管理/chap0x04_resolve/script/worldcupplayerinfo.tsv)
...
function counttage()
{
   ...
   do
     ...
     if [ "$a" != "Age" ];then
        ...
        if [ "$a" -gt $oldest ];then
           oldest=$a
           oldestname=$(awk -F '\t' 'NR=='$[$total_sum +1]' {print $9}' Linux系统与网络管理/chap0x04_resolve/script/worldcupplayerinfo.tsv)
        elif [ "$a" -lt $youngest ];then
           youngest=$a
           youngestname=$(awk -F '\t' 'NR=='$[$total_sum +1]' {print $9}' Linux系统与网络管理/chap0x04_resolve/script/worldcupplayerinfo.tsv)
        fi
     fi
    done
